...
class InboxViewMixin:
    def _build_inbox_view(self, parent: ctk.CTkFrame) -> None:
        ...
        search.grid(row=0, column=2, columnspan=2, sticky="ew", padx=(0, 8))
        search.bind("<Return>", lambda _event: self.refresh_inbox())
        ctk.CTkButton(toolbar, text="Tìm kiếm", width=92, command=self.refresh_inbox).grid(row=0, column=4, padx=4)
        ctk.CTkButton(toolbar, text="Quét tất cả", width=104, command=self.start_scan).grid(row=0, column=5, padx=(4, 0))
        ...
        pane = tk.PanedWindow(parent, orient="horizontal", sashwidth=6, relief="flat", borderwidth=0)
        pane.grid(row=2, column=0, sticky="nsew", padx=20, pady=(0, 18))
        self.inbox_pane = pane

        list_frame = ctk.CTkFrame(pane, corner_radius=6)
        detail_frame = ctk.CTkFrame(pane, corner_radius=6)
        pane.add(list_frame, minsize=500, stretch="always")
        pane.add(detail_frame, minsize=300, stretch="always")
        ...
        self.inbox_tree = self._create_tree(list_frame, columns, headings, widths)
        self.inbox_tree.bind("<<TreeviewSelect>>", self.on_message_selected)

        ctk.CTkLabel(detail_frame, text="Nội dung mail", font=ctk.CTkFont(size=16, weight="bold")).pack(
            anchor="w", padx=14, pady=(12, 8)
        )
        self.message_text = ctk.CTkTextbox(detail_frame, wrap="word", font=("Segoe UI", 13), corner_radius=4)
