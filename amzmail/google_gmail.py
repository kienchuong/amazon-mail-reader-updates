...
def interactive_google_login(client_id: str, timeout_seconds: int = 300) -> GoogleLogin:
...
def _access_token(account, db, client_id: str) -> str:
    try:
    ...
    refreshed = _post_form({"client_id": client_id, "refresh_token": refresh_token, "grant_type": "refresh_token"})
...
def _message(account, db, client_id: str, message_id: str, fmt: str) -> dict:
    token = _access_token(account, db, client_id)
    return _get(f"{GMAIL_ROOT}/messages/{urllib.parse.quote(message_id, safe='')}?format={fmt}", token)


def fetch_google_body(account, db, client_id: str, message_id: str) -> str:
    return _body_text(_message(account, db, client_id, message_id, "full").get("payload") or {})


def test_google_connection(account, db, client_id: str) -> None:
    _get(f"{GMAIL_ROOT}/profile", _access_token(account, db, client_id))
    db.set_connection_status(int(account["id"]), "Đã kết nối")


def scan_google_account(account, db, client_id: str, days_back: int, max_messages: int, include_general: bool) -> ScanResult:
    scanned = saved = 0
    try:
        token = _access_token(account, db, client_id)
        ...
        for item in listing.get("messages") or []:
            ...
            if not message_id:
                continue
            metadata = _message(account, db, client_id, message_id, "metadata")
            ...
            if not include_general and not looks_interesting(from_addr, subject):
                continue
            scanned += 1
            full = _message(account, db, client_id, message_id, "full")
