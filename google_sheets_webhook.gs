...
<html><head><base target="_top"><meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">
...
function token(){return localStorage.getItem('amz_mobile_token')||''}function run(name,...args){return new Promise((resolve,reject)=>google.script.run.withSuccessHandler(resolve).withFailureHandler(reject)[name](...args))}
...
async function load(){try{snapshot=await run('mobileSnapshot',token());render()}catch(e){localStorage.removeItem('amz_mobile_token');login()}}
...
const root=document.querySelector('#root');token()?load():login();
</script></body></html>`;
