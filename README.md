...
2. Mở tab `Cài đặt`, nhập Client ID rồi bấm `Lưu cấu hình`.
3. Mở tab `Accounts`, chọn `Outlook` và bấm `Đăng nhập Microsoft`.
...
- Tab `Inbox`: quét, tìm kiếm và lọc Payment, Reject, Amazon Account, Security.
- Nội dung đầy đủ chỉ được tải khi cần và không lưu lâu dài trong database.
- Tab `Payment`: tổng hợp riêng theo từng loại tiền, xuất CSV hoặc Google Sheet.
...
Tab `Cài đặt` dùng sẵn kho `kienchuong/amazon-mail-reader-updates`. Mỗi release cần có:
...
Yêu cầu Python 3.12 trên Windows có Tkinter và thư viện trong `requirements.txt`. File `run_app.bat` ưu tiên Python đi kèm Codex, sau đó tự tìm Python đã cài trên Windows.

## Cấu trúc chính

- `app.py`: khởi động ứng dụng.
- `amzmail/ui.py`: toàn bộ giao diện Windows.
